...
Now let's switch to a more relevant example using the `origami` package. In this next cell, we'll write a function that can create a new Noteable Notebook and start a kernel for us.
...
async def run_conversation(
    ...
    if "function_call" not in response_message.keys():
        # we got a text/content response; print it out for easy viewing and exit cleanly
...
In this post, we touched on the OpenAI plugins and how to mimic similar behavior with OpenAI's function calling API, wrote some basic function schemas, and got our feet wet with the `origami` package by creating a notebook and starting a kernel using
