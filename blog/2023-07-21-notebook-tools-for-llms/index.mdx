...
I've created new notebook named <a>IMDb_ETL_Notebook.ipynb</a>.
...
What we needed was a way for the LLM to be able to create and edit cells, start new notebooks, and debug its own code. Everything that would be part of the workflow for a data scientist, data engineer, or anyone else looking to learn in a notebook.

## Noteable's Headless Execution

Building on top of our foundations for [realtime collaboration and scheduled notebooks](https://github.com/noteable-io/papermill-origami), we began work on a plugin that would let the model create and edit cells, start new notebooks, and read outputs. The LLM could participate in the full workflow of data science-everything from data loading to exploration to reporting.

The reason the model can do this is that we built a realtime API for users to collaborate and for notebooks to run serverside as scheduled jobs. This also means that **AI Agents can join the mix**. Let's look at how this happens from a ChatGPT plugin perspective.
...
As soon as you have data loaded, start asking questions and telling ChatGPT to give it a shot.
...
One of my favorite approaches from here is to start asking questions of my data. ChatGPT can debug, reason about outputs and exceptions, and generally be a great partner in tandem with the Noteable Plugin.

## Contrasting with OpenAI's Code Interpreter

I value different tools for different jobs. And often, the difference between these tools is not the functionality offered, but the interface and environment provided. In both Noteable and OpenAI's Code Interpreter, the underlying Large Language Model can reason, debug, run code, analyze data, and more.

One of the strengths of OpenAI's Code Interpreter is that it's fast to start up. They've used clever prompting and Reinforcement Learning from Human Feedback (RLHF) to create a compelling user experience.

However, one aspect of the Code Interpreter's design limits its capability: it's ephemeral. This approach allows for aggressive caching, but it also means you lose your work when your session ends.

I know this from [creating tmpnb back in 2014](https://speakerdeck.com/rgbkrk/tmpnb-at-pytexas?slide=10), which we used to [provide notebooks to readers of an IPython article in the Nature Journal](https://www.nature.com/articles/515151a). Not long after we had this deployed for everyone, we started finding out that people wanted to use our temporary notebooks for real analysis work. We fundamentally disrupted the problem of _access_ and people wanted _more_.

In case anyone at OpenAI reads this, **thank you so much for the models and the absolutely stunning APIs**. I'm not even getting a chance to gush about Chat Functions yet. Another time.

## Providing Durable Access with Notebooks

For some people, that ephemeral nature is perfect. For others, they need a more durable solution—this is where a Notebook Platform like Noteable comes in. I enjoy all the access to my data files, environment variables/secrets, and databases.
...
While LLMs are powerful standalone tools, they can be even more powerful when used in robust environments like notebooks. I've always been excited to enable more people to feel the delight of interactive computing. Now it feels like that reach is much more broad and more accessible.

My goal is to build great environments for collaborative analysis and development. To enable everyone, from small teams to massive scale. We're seeing a glimpse of this now with Noteable and OpenAI's GPT models. I can't wait to see what else we build.
